...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.editOneOrder') }}" class="form-order">
        {{ csrf_field() }}
        ...
        <div class="form-group">
            <label for="mark" class="lab">Время доставки<br/>
                <input type="time" class="form-control" id="TimeDelivery" list="time-list" name="TimeDelivery" placeholder="Время доставки"
                       value="{{ $result['TimeDelivery']  or '' }}">
            </label>
        </div>
